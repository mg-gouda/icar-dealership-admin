...
export default function AppointmentDetailPage() {
  ...
  if (!appt) return <div className="p-6 text-red-400 text-sm">Appointment not found.</div>;
  ...
  return (
    <div className="p-6 max-w-2xl">
      <div className="flex items-center gap-3 mb-6">
        <Link href="/appointments" className="text-gray-500 hover:text-white text-xs transition">← Appointments</Link>
        <h1 className="text-xl font-semibold text-white">{appt.type.replace(/_/g, ' ')}</h1>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="rounded-xl border border-white/5 bg-gray-900 p-5 space-y-3">
          ...
          <div>
            ...
            <p className="text-white text-sm">{appt.type.replace(/_/g, ' ')}</p>
          </div>
          <div>
        ...
        <div className="mt-4 rounded-xl border border-white/5 bg-gray-900 p-5">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-500 font-mono mt-0.5">VIN: {appt.vehicle.vin}</p>
            </div>
            <Link href={`/vehicles/${appt.vehicle.id}`}
              className="text-xs text-blue-400 hover:text-blue-300 transition">View vehicle →</Link>
          </div>
        </div>
      )}
    </div>
