...
function statusLabel(s: string, isAr: boolean) {
  if (s === 'RECONCILED') return isAr ? 'مسوى' : 'Reconciled';
  if (s === 'IN_PROGRESS') return isAr ? 'جاري' : 'In Progress';
...
export default function BankStatementsPage() {
  ...
  const { isAr } = useLang();
  const { data: statements, loading, error, reload } = useQuery<BankStatement[]>('/finance/bank-statements');
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-semibold text-white">{isAr ? 'كشوف البنك' : 'Bank Statements'}</h1>
          <p className="text-xs text-gray-500 mt-0.5">{isAr ? 'استيراد وإدارة كشوف الحساب البنكية' : 'Import & reconcile bank transactions'}</p>
        </div>
        <button onClick={() => setShowCreate(true)}
          className="px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white text-xs font-medium rounded-lg transition">
          {isAr ? '+ كشف جديد' : '+ Statement'}
        </button>
      </div>
      ...
      <div className="rounded-xl border border-white/5 bg-gray-900 overflow-hidden">
        <table className="w-full text-sm">
          <thead className="border-b border-white/5 text-xs text-gray-500">
            <tr>
              <th className="px-4 py-3 text-left font-medium">{isAr ? 'المرجع' : 'Reference'}</th>
              <th className="px-4 py-3 text-left font-medium">{isAr ? 'الحساب البنكي' : 'Bank Account'}</th>
              <th className="px-4 py-3 text-left font-medium">{isAr ? 'التاريخ' : 'Date'}</th>
              <th className="px-4 py-3 text-right font-medium">{isAr ? 'الرصيد الافتتاحي' : 'Opening'}</th>
              <th className="px-4 py-3 text-right font-medium">{isAr ? 'الرصيد الختامي' : 'Closing'}</th>
              <th className="px-4 py-3 text-center font-medium">{isAr ? 'الحركات' : 'Lines'}</th>
              <th className="px-4 py-3 text-left font-medium">{isAr ? 'الحالة' : 'Status'}</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-white/5">
            {stmts.map((s) => (
              <tr key={s.id} onClick={() => router.push(`/finance/bank-statements/${s.id}`)} className="hover:bg-white/5 cursor-pointer transition">
                ...
                <td className="px-4 py-2.5 text-gray-400 text-xs">{fmtDate(s.date, isAr)}</td>
                ...
                <td className="px-4 py-2.5">
                  ...
                  }`}>{statusLabel(s.status, isAr)}</span>
                </td>
              </tr>
            ))}
            {stmts.length === 0 && !loading && (
              <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-600 text-sm">{isAr ? 'لا توجد كشوف بنكية.' : 'No bank statements imported.'}</td></tr>
            )}
          </tbody>
        </table>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md rounded-2xl bg-gray-900 border border-white/10 shadow-2xl">
            <div className="flex items-center justify-between p-5 border-b border-white/5">
              <h2 className="text-sm font-semibold text-white">{isAr ? 'كشف جديد' : 'New Bank Statement'}</h2>
            ...
            <form onSubmit={create} className="p-5 space-y-3">
              <SearchableCombobox label={isAr ? 'الحساب البنكي *' : 'Bank Account *'} options={baOpts} value={form.bankAccountId} onChange={(v) => set('bankAccountId', v)} placeholder={isAr ? 'اختر حسابًا بنكيًا…' : 'Select bank account…'} />
              <div><label className="block text-xs text-gray-500 mb-1">{isAr ? 'المرجع *' : 'Reference *'}</label>
              ...
              <div><label className="block text-xs text-gray-500 mb-1">{isAr ? 'التاريخ' : 'Date'}</label>
