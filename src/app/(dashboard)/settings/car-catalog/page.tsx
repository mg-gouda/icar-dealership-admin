...
export default function CarCatalogPage() {
  ...
  async function addMake(e: React.FormEvent) {
    ...
    setBusy(true); setErr('');
    try {
      await apiFetch('/part-catalog/makes', {
        method: 'POST',
        body: JSON.stringify({ name: addMakeName.trim() }),
      });
      setAddMakeName('');
      reloadMakes();
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <h1 className="page-title">{isAr ? 'كتالوج السيارات (القطع)' : 'Car Catalog'}</h1>
          <p className="page-subtitle">
            {isAr ? 'إدارة الماركات والطرازات لتوافق قطع الغيار' : 'Manage makes & models for parts fitment'}
          </p>
        </div>
      </div>
      ...
      <div className="page-body">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: selectedMake ? '1fr 1fr' : '420px', gap: '1.25rem' }}>
          ...
          <div className="card" style={{ overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <div style={{
              ...
              {isAr ? 'الماركات' : 'Makes'} ({makes.length})
            </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', maxHeight: 460 }}>
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      cursor: 'pointer',
                      background: isSelected ? 'color-mix(in srgb, var(--primary) 8%, transparent)' : undefined,
                    }}
                  >
                    <div>
                    ...
                    <button
                      ...
                      title={canDelete
                        ...
                        : (isAr ? 'لا يمكن الحذف — يحتوي على طرازات' : 'Cannot delete — has models')}
                      style={{
                        ...
                        opacity: canDelete ? 1 : 0.35, fontSize: '0.85rem', lineHeight: 1,
                      }}
                    >
                      ×
                    </button>
              ...
              {makes.length === 0 && !makesLoading && (
                <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-3)', fontSize: '0.8rem' }}>
                  {isAr ? 'لا توجد ماركات بعد. أضف واحدة أدناه.' : 'No makes yet. Add one below.'}
                </div>
              )}
            </div>
            ...
            >
              ...
              <button
                ...
                disabled={busy || !addMakeName.trim()}
              >
                {isAr ? '+ إضافة' : '+ Add'}
              </button>
            </form>
          ...
          {/* Models panel — shown when a make is selected */}
          {selectedMake && (
            <div className="card" style={{ overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
              <div style={{
                padding: '0.75rem 1rem', borderBottom: '1px solid var(--border)',
                ...
                {selectedMake.name} — {isAr ? 'الطرازات' : 'Models'} ({models.length})
              </div>
              ...
              <div style={{ flex: 1, overflowY: 'auto', maxHeight: 460 }}>
                {models.map((model) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      disabled={busy}
                      title={isAr ? 'حذف الطراز' : 'Delete model'}
                      style={{
                        background: 'none', border: '1px solid var(--border)', borderRadius: '0.375rem',
                ...
                {models.length === 0 && !modelsLoading && (
                  <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-3)', fontSize: '0.8rem' }}>
                    {isAr
                      ? `لا توجد طرازات لـ ${selectedMake.name}. أضف طرازاً أدناه.`
                      : `No models for ${selectedMake.name}. Add one below.`}
                  </div>
                )}
              </div>
              ...
              >
                ...
                <button
                  type="submit"
                  className="btn btn-primary btn-sm"
                  disabled={busy || !addModelName.trim()}
                >
                  {isAr ? '+ إضافة' : '+ Add'}
                </button>
              </form>
